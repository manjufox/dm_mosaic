...
class ImageProcessing:
    ...
    def set_image(self, x):
        ...
        x = self.to_normalized_tensor(x)
        self.value.update({"image": x,"eval_img":x,"train_img":x,"encoder_input":x})
    ...
    def to_normalized_tensor(self,x=None):
        if x is None:
            x = self.value["image"]
        x = v2.Compose([
    ...
    def de_normalize(self,x=None):
        ...
        return v2.Compose(
            [
                ...
                v2.Lambda(lambda x:torch.clamp(x,0,255)),
                # v2.Lambda(lambda x:x.to(torch.uint8)),
            ]
        )(x)
...
if __name__ == "__main__":
    ...
    pprint(f"data ->{data["train_img"].shape,data["image"].shape}")
    display_images([data["train_img"],data["image"]],nrow=conf.batch_size)
